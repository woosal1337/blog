...
export const meta = {
	...
	seoTitle: "Why 95% of Enterprise AI Projects Fail to Deliver Value",
	date: "2026-05-17",
	summary:
	...
	seoDescription:
		"Why most enterprise AI projects fail despite stronger models—and how operational context, agent infrastructure, and workflow design change the outcome.",
	socialDescription:
		"Why stronger models do not fix enterprise AI—and how operational context and agent infrastructure change outcomes.",
	...
	coverAlt: "Contour artwork for an analysis of why enterprise AI projects fail",
};

export const metadata = createPostMetadata(
	meta,
	"the-95-percent-failure-rate-is-not-about-the-models",
);

<PostMeta meta={meta} />
<ArticleJsonLd meta={meta} slug="the-95-percent-failure-rate-is-not-about-the-models" />
...
|---|---|---|
| [MIT NANDA, *The GenAI Divide*](https://nanda.media.mit.edu/ai_report_2025.pdf) | July 2025 | 5% of integrated AI pilots pull millions in value, the other 95% have nothing to show |
| [BCG](https://www.bcg.com/publications/2024/wheres-value-in-ai) | 2024 | 4% have hit AI value at scale |
| [Deloitte](https://www.deloitte.com/global/en/issues/ai/ai-roi-the-paradox-of-rising-investment-and-elusive-returns.html) | 2025 survey | 6% achieve AI ROI within a year |
| [RAND](https://www.rand.org/content/dam/rand/pubs/research_reports/RRA2600/RRA2680-1/RAND_RRA2680-1.pdf) | 2024 | 80%+ of AI projects fail, twice the rate of normal IT projects |
| [IBM](https://newsroom.ibm.com/2025-05-06-ibm-study-ceos-double-down-on-ai-while-navigating-enterprise-hurdles) | 2025 | 75% of AI initiatives have not delivered expected ROI |
| [McKinsey](https://www.mckinsey.com/capabilities/quantumblack/our-insights/the-state-of-ai-how-organizations-are-rewiring-to-capture-value) | 2025 | 78% of orgs use AI regularly, 80%+ report zero EBIT impact |
...
| Source | Finding |
|---|---|
| [GitHub Copilot study, 2022](https://github.blog/news-insights/research/research-quantifying-github-copilots-impact-on-developer-productivity-and-happiness/) | 55% faster on real tasks. 1h 11m vs 2h 41m on the same work. |
| [Anthropic internal study, 2025](https://www.anthropic.com/research/how-ai-is-transforming-work-at-anthropic) | 132 engineers and researchers, 200k Claude Code transcripts, 50% self-estimated productivity gain. |
| [Anthropic productivity study, 2025](https://www.anthropic.com/research/estimating-productivity-gains) | 100k Claude.ai conversations, 80% estimated reduction in task time. |
| [Sundar Pichai, April 2026](https://blog.google/innovation-and-ai/infrastructure-and-cloud/google-cloud/cloud-next-2026-sundar-pichai/) | 75% of new code at Google is AI-generated and engineer-approved. The number was 50% the previous fall. |
...
The labs ([OpenAI](https://openai.com/), [Anthropic](https://www.anthropic.com/), [Cursor](https://cursor.com/)) poured everything they had into engineering because that is where the four properties hold. They got engineering working. They have not solved the rest, because the rest does not have the substrate.
...
This is the same conclusion the harness-engineering literature has reached from a different angle. [LangChain's Terminal Bench 2.0 comparison](https://www.langchain.com/blog/tuning-deep-agents-different-models) moved the same model from 52.8% to 66.5% with a different harness. That is a 14-point swing from the harness alone. *If you are not the model, you are the harness.* The harness is the operational layer. The operational layer is the moat.
...
[OpenAI shipped Codex Cloud](https://openai.com/index/introducing-upgrades-to-codex/), which is OpenAI doing the operational fieldwork inside customer organizations. Anthropic shipped [managed agents](https://www.anthropic.com/engineering/managed-agents) and [Claude Design](https://www.anthropic.com/news/claude-design-anthropic-labs), which are Anthropic doing the same thing in different surfaces. [Cursor](https://cursor.com/)'s ARR moved past Anthropic's API revenue because Cursor is not selling the model, it is selling the harness around the model. [Sierra](https://sierra.ai/) is doing the same thing on the customer-experience surface, the entire product is context engineering as a service, with [Ghostwriter](https://sierra.ai/product/ghostwriter) generating context blocks and conditions from existing SOPs, [Journeys and the Agent SDK](https://sierra.ai/blog/context-engineering-the-key-to-great-agents) giving operators a no-code editor and a path for teams that want to manage the layer as code. Every model lab and every serious agent vendor has now figured out that selling the model alone is not enough. Selling the runtime alone is not enough. You need the model plus the runtime plus the context engineering discipline plus a team that goes and embeds inside the enterprise and figures out what to actually build for it.
...
Sources. Vasuman, [the operational layer is the moat](https://x.com/vasuman/status/2049659161005470071). Neil Rahilly, [context engineering, the key to great agents](https://x.com/neilrahilly/status/2051723442928664620). The 5% / 95% framing originates with MIT NANDA's GenAI Divide report from July 2025. Convergence across BCG, Deloitte, RAND, IBM, and McKinsey gives the number its weight.

<RelatedPosts currentSlug="the-95-percent-failure-rate-is-not-about-the-models" />
