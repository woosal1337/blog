...
export const meta = {
	...
	seoTitle: "Ears Without Eyes: Efficient Video Analysis for AI Agents",
	date: "2026-04-20",
	summary:
		"An agent watching video should default to listening, and only open its eyes when audio admits it missed something.",
	seoDescription:
		"A cheaper multimodal-agent architecture that listens to video first, detects transcript gaps, and uses vision only where audio is insufficient.",
	socialDescription:
		"A multimodal agent architecture that listens to video first and uses vision only where the transcript has gaps.",
	tags: ["ai", "agents", "multimodal", "video"],
	cover: "/blog-cover/ears-without-eyes",
	coverAlt: "Contour artwork for efficient multimodal video analysis by AI agents",
};

export const metadata = createPostMetadata(meta, "ears-without-eyes");

<PostMeta meta={meta} />
<ArticleJsonLd meta={meta} slug="ears-without-eyes" />
...
Audio at the same length is a single transcription pass. [Whisper](https://github.com/openai/whisper) runs on a local machine in seconds on a small model. No per-token cost, no rate limits, no API quota burning down in real time. The asymmetry between the two channels is at least three orders of magnitude, sometimes four, depending on which vision model you route to.
...
<RelatedPosts currentSlug="ears-without-eyes" />
