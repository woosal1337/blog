...
export const meta = {
	...
	seoTitle: "How I Built a Parallel AI Advisory Board with Claude Code",
	date: "2026-05-09",
	summary:
		"meclis. Three named thinkers arguing in your browser. The consulting bundle, unbundled.",
	seoDescription:
		"How I built meclis: a local Claude Code workflow that dispatches opinionated AI advisors in parallel and visualizes their debate in the browser.",
	socialDescription:
		"How meclis dispatches opinionated AI advisors in parallel through Claude Code and visualizes their debate.",
	tags: ["ai", "agents", "claude-code", "meclis"],
	cover: "/blog-cover/i-cloned-my-advisory-board",
	coverAlt: "Contour artwork for an AI advisory board built with Claude Code",
};

export const metadata = createPostMetadata(meta, "i-cloned-my-advisory-board");

<PostMeta meta={meta} />
<ArticleJsonLd meta={meta} slug="i-cloned-my-advisory-board" />
...
The entire stack costs nothing extra to run on top of the [Claude Code](https://code.claude.com/docs/en/overview) subscription I already have. Paul Graham, Seth Godin, Robert Greene. Each one of them in their own voice, with their own frameworks, taking my actual question and pulling it apart from three different angles. They thought about it for nine seconds, six seconds, fourteen seconds. They disagreed in interesting ways. The synthesis at the end was sharper than what I had walked in with.
...
meclis is a 2D pixel-art scene in the browser. You type `/meclis <your question>` in Claude Code. The skill dispatches one [sub-agent](https://code.claude.com/docs/en/sub-agents) per advisor in parallel, each loaded with a markdown character pack describing that advisor's voice, frameworks, and quotes. Three local hooks installed in your Claude Code settings mirror those dispatches into a tiny local server. The server broadcasts events over [SSE](https://html.spec.whatwg.org/multipage/server-sent-events.html) to a viewer at `localhost:5173`. The advisors appear as standing characters on a Greek symposium stage. Each one sits in a thinking pose, then speaks. A speech bubble fills in. A codex panel on the right keeps the full transcript.
...
The frontend is [Vite](https://vite.dev/) plus [React](https://react.dev/) plus [TypeScript](https://www.typescriptlang.org/) plus [PixiJS](https://pixijs.com/). The Pixi canvas runs the symposium scene, three character sprites on marble discs, parchment speech bubbles that type out as the advisor's response streams in, a thought-bubble cloud during the thinking phase, click-to-focus to spotlight whichever speaker you want to read closely.

The backend is [Node](https://nodejs.org/en) plus [Hono](https://hono.dev/). An in-memory event bus, an SSE broadcaster, a small set of HTTP routes. `POST /api/meclis/hooks/event` for incoming hook events. `GET /api/meclis/stream` for outgoing SSE. `POST /api/meclis/reset` for clearing a session. About four hundred lines total.

The bridge between Claude Code and meclis is three [hook scripts](https://code.claude.com/docs/en/hooks). `PreToolUse` fires when an agent sub-agent is dispatched. `PostToolUse` fires when it returns. `SubagentStop` fires when the wrapping skill exits. The hook script reads the sub-agent's prompt, identifies which character pack it matches by looking for the h1 line or "answering AS ", and posts a small event to the local server. If the prompt does not match a known advisor, the hook silently skips. Nothing in Claude Code is blocked, nothing waits on the network.

The advisor sprites were generated with [Gemini Nano Banana Pro](https://ai.google.dev/gemini-api/docs/models/gemini-3-pro-image) using two reference sprites as visual style guides. The pipeline auto-crops the alpha channel and rescales each new sprite so the figure fills the canvas at the same proportion as the references. The same pipeline generates the readme banner and the article banner you are looking at.
...
The repository is open source under MIT. The readme is the entry point. The advisor authoring guide tells you how to write a new pack. The hook installer is idempotent and migrates legacy markers automatically. Setup is two minutes if you already have Node and [pnpm](https://pnpm.io/).
...
<RelatedPosts currentSlug="i-cloned-my-advisory-board" />
